...
- API-key precedence is `--api-key` → `EXA_API_KEY` → stored credential. `exa auth login|status|logout` manages macOS Keychain, Linux Secret Service (`secret-tool`), or a Windows DPAPI-encrypted file without native addons. Helper calls time out after three seconds and never place the secret in argv. Secure-storage failure is fatal unless login explicitly receives `--insecure-storage`; that fallback is an atomic `0600` plaintext file.
